...
try {
  ...
  const alice = await session('alice'); const approver = await session('approver'); const reviewer = await session('designReviewer'); const authorReviewer = await session('designAuthorReviewer')
  ...
  const nonReviewer = await post(alice, `/v1/workspaces/${workspace}/change-cases/${id}/design-decision`, { expectedVersion: version, designDigest: digest1, decision: 'APPROVED', rationale: 'not a reviewer' }); assert.equal(nonReviewer.status, 403); assert.equal(nonReviewer.body.code, 'CAPABILITY_MISSING')
