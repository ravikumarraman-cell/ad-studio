...
export function FeatureDelivery() {
  ...
  const modal = showCreate ? <CreateCaseModal workspace={workspace} error={createError} onClose={() => { setShowCreate(false); setCreateError('') }} onError={setCreateError} onCreated={() => { setShowCreate(false); refresh() }}/> : showImport ? <ImportFeaturesModal workspace={workspace} onClose={() => setShowImport(false)} onDone={() => { setShowImport(false); refresh() }}/> : null
  return <RealWorkspace principal={session.data.principal} memberships={memberships} activeWorkspace={activeWorkspace} setWorkspaceId={setWorkspaceId} changeCases={changeCases.data?.changeCases ?? []} loading={changeCases.isFetching} error={Boolean(changeCases.error)} onRefresh={refresh} onCreate={() => setShowCreate(true)} onImport={() => setShowImport(true)} modal={modal}/>
}
