...
export function ImportFeaturesModal({ workspace, onClose, onDone }: { workspace?: Membership; onClose: () => void; onDone: () => void }) {
  const [rows, setRows] = useState<ImportRow[]>([]); const [message, setMessage] = useState(''); const [importId, setImportId] = useState(() => crypto.randomUUID()); const valid = rows.filter((row) => row.feature && !row.errors.length)
  ...
  const confirm = async () => {
    if (!workspace || !valid.length) return
