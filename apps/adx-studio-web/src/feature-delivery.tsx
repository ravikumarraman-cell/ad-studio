...
import { consumeWorkspaceAfterOptumSignIn, signInWithOptum } from './entra-auth'
...
export function FeatureDelivery() {
  ...
  useEffect(() => {
    if (mode !== 'real' || !session.error || sessionStorage.getItem(automaticOptumRecoveryKey) === '1') return
  ...
  useEffect(() => { if (session.data) sessionStorage.removeItem(automaticOptumRecoveryKey) }, [session.data])
