...
export function createPreviewGitProvider({ providerId, repositories }) {
  ...
  for (const repository of repositories) {
    const repositoryId = typeof repository?.repositoryId === 'string' ? repository.repositoryId.trim() : ''
    const canonicalRemote = typeof repository?.canonicalRemote === 'string' ? repository.canonicalRemote.trim() : ''
    const defaultBaseRef = typeof repository?.defaultBaseRef === 'string' ? repository.defaultBaseRef.trim() : ''
    const projectPath = normalizeProjectPath(repository?.projectPath)
    if (!repositoryId || !canonicalRemote.startsWith('https://') || !defaultBaseRef.startsWith('refs/heads/')) throw new ChangeCaseError('GIT_REPOSITORY_REGISTRATION_INVALID', 'Registered repositories require an ID, canonical HTTPS remote, and heads base ref.')
    if (registered.has(repositoryId)) throw new ChangeCaseError('GIT_REPOSITORY_REGISTRATION_DUPLICATE', 'Preview provider repository IDs must be unique after normalization.')
    registered.set(repositoryId, Object.freeze({ repositoryId, canonicalRemote, defaultBaseRef, projectPath }))
  }
  return Object.freeze({ providerId: providerId.trim(), mode: 'PREVIEW_ONLY', capabilities: Object.freeze({ branchPreview: true, commitPreview: true, pullRequestPreview: true, ciTrigger: false, reviewFindingIngestion: false, merge: false }), repository(repositoryId) { const requestedId = typeof repositoryId === 'string' ? repositoryId.trim() : ''; const repository = registered.get(requestedId); if (!repository) throw new ChangeCaseError('GIT_REPOSITORY_DENIED', `The retained Intake target repository ${requestedId || '(missing)'} is not registered for this preview provider. Add that exact repositoryId to ADX_PREVIEW_GIT_REPOSITORIES_JSON.`, { details: { repositoryId: requestedId || null } }); return repository } })
}
...
export function createPreviewDeliveryPlan({ provider, changeCaseId, repositoryId, baseRef, candidateDigest, evidenceDigest, changes, title }) {
  ...
  const normalizedChanges = normalizeChanges(changes); const changeDigest = sha256(normalizedChanges); const branch = `adx/preview/${changeCaseId}`; const commitDigest = sha256({ schema: 'adx-preview-commit-v1', repository: repository.canonicalRemote, projectPath: repository.projectPath, baseRef: requestedBaseRef, candidateDigest, evidenceDigest, changes: normalizedChanges }); const pullRequestDigest = sha256({ schema: 'adx-preview-pr-v1', repository: repository.canonicalRemote, branch, baseRef: requestedBaseRef, commitDigest, title: title.trim() })
...
function normalizeProjectPath(value) { if (value === undefined || value === null || value === '') return null; const path = typeof value === 'string' ? value.trim() : ''; if (!path || path.startsWith('/') || path.includes('\\') || path.split('/').some((part) => !part || part === '.' || part === '..')) throw new ChangeCaseError('GIT_REPOSITORY_REGISTRATION_INVALID', 'A registered project path must be a canonical relative directory path.'); return path }
