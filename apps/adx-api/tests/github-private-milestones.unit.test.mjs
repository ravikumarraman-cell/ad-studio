...
test('private GitHub milestone reads use only the server credential and exclude pull requests', async () => {
  ...
  const client = createPrivateGitHubMilestoneClient({ token: 'server-only-token', fetchImpl: async (url, init) => {
    ...
    return response([{ number: 10, title: 'Add medication reminder', body: 'Give people a local reminder.', labels: [{ name: 'health-x' }], html_url: 'https://github.com/acme/health-x/issues/10' }, { number: 11, title: 'Pull request', pull_request: {} }])
  } })
