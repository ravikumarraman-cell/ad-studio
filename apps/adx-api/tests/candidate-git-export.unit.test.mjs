...
test('exports only content-addressed candidate changes from a registered source checkout', async () => {
  ...
  const commands = []
  const git = async (_cwd, argumentsList) => { commands.push(argumentsList); return argumentsList[0] === 'rev-parse' ? 'abc123\n' : 'https://github.com/example/repository.git\n' }
...
test('tolerates a dirty source checkout while still exporting the verified candidate changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'adx-git-export-'))
  const source = join(root, 'source'); const candidate = join(root, 'candidate')
  await mkdir(source); await mkdir(candidate)
  ...
  try {
    const exported = await createCandidateGitExport({
      ...
      canonicalRemote: 'https://github.com/example/repository',
      runGit: git,
    })
...
test('exports only the registered project subtree despite unrelated source changes', async () => {
  ...
  const commands = []; const git = async (_cwd, argumentsList) => { commands.push(argumentsList); return argumentsList[0] === 'rev-parse' ? 'abc123\n' : 'https://github.com/example/repository.git\n' }
  try {
