...
function service({ evidence = [{ status: 'PASS', ...binding }], timeline = [{ eventType: 'ChangeCaseVerificationCompleted.v1', payload: binding }], repositories = [{ repositoryId: 'health-x', canonicalRemote: 'https://example.test/health-x.git', defaultBaseRef: 'refs/heads/main', projectPath: 'apps/health-x' }], targetRepository = 'health-x' } = {}) {
  const retained = []
  const exports = []
  return {
    retained, exports,
    prepare: createPreviewDeliveryService({
      ...
      changeCaseRepository: { intakeView: async () => ({ intent: { targetRepository } }) },
      servicePrincipal: { type: 'service', id: 'delivery-preview-service' },
      sourceRoot: '/source',
      candidateRoot: '/candidate',
      createExport: async (input) => { exports.push(input); return { baseCommit: 'a'.repeat(40), exportDigest: 'sha256:export', changes: [{ path: 'apps/health-x/src/feature.js', afterDigest: 'sha256:feature' }] } },
...
test('prepares a server-owned preview-only plan from the exact Gate D binding', async () => {
  ...
  assert.deepEqual(prepared.exports, [{ sourceRoot: '/source', candidateRoot: '/candidate', candidateDigest: binding.candidateDigest, canonicalRemote: 'https://example.test/health-x.git', projectPath: 'apps/health-x' }])
})
...
test('reports the exact retained Intake repository required by a mismatched provider registration', async () => {
  const prepared = service({ targetRepository: 'health-x', repositories: [{ repositoryId: ' ad-studio ', canonicalRemote: 'https://example.test/ad-studio.git', defaultBaseRef: 'refs/heads/main' }] })
  await assert.rejects(prepared.prepare, (error) => error.code === 'GIT_REPOSITORY_DENIED' && error.details.repositoryId === 'health-x' && error.message.includes('health-x'))
})

test('normalizes whitespace around a registered repository ID', async () => {
  const prepared = service({ repositories: [{ repositoryId: ' health-x ', canonicalRemote: ' https://example.test/health-x.git ', defaultBaseRef: ' refs/heads/main ' }] })
  const result = await prepared.prepare
  assert.equal(result.previewPlanId, 'plan-1')
  assert.equal(prepared.retained[0].plan.repository.repositoryId, 'health-x')
})
