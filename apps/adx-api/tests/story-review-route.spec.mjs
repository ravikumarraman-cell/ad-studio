...
test('story review deep link presents authoritative retained intent, risk, and BDD stories', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.getByRole('status', { name: '' }).filter({ hasText: 'Applied specification: story-spec-delivery-slices v10.10 (sha256:story-spec)' })).toBeVisible()
    const selectAllStories = page.getByRole('checkbox', { name: 'Select all stories' })
    await selectAllStories.check()
    await expect(page.locator('#suggestion-list input[type="checkbox"]')).toBeChecked()
    await expect(page.locator('.story-card')).toHaveCount(1)
    await expect(page.getByRole('button', { name: /Accept selected suggestions \(1\)/ })).toBeEnabled()
    await selectAllStories.uncheck()
    await expect(page.getByRole('button', { name: 'Accept selected suggestions' })).toBeDisabled()
    await selectAllStories.check()
    ...
    await expect(page.locator('.story-card label').first()).toHaveCSS('color', 'rgb(36, 71, 63)')
    await expect(page.locator('.story-card input').first()).toHaveCSS('border-top-width', '1px')
    let submitted
