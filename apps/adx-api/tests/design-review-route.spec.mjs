...
test('design review deep link explains retained artifacts, residual risk, and the safe next action', async ({ page, request }, testInfo) => {
  ...
  let changeCaseId
  try {
    const created = await request.post(`/v1/workspaces/${workspace}/change-cases`, { headers: headers(alice), data: { title: changeCaseTitle, riskTier: 'R2' } }); const createdBody = await created.json(); changeCaseId = createdBody.changeCaseId; const { projectionVersion: createVersion } = createdBody
