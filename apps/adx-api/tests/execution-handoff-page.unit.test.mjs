...
test('execution handoff sends only a reviewed coding specification ID with dispatch', () => {
  const page = renderExecutionHandoffPage(changeCase, { ...options, templates: [{ id: 'evidence-first-feature', label: 'Evidence-first feature', description: 'Bounded feature delivery.', guidance: 'Implement only approved scope.' }] })
...
test('execution handoff presents durable live run status without exposing unvalidated model output', () => {
  ...
  assert.match(page, /A code preview becomes available only after the candidate passes validation/)
})