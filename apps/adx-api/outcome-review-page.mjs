...
export function outcomeReviewPage(changeCase, records, options = {}) {
  ...
  const next =
    isReadyForDelivery && records.length
      ? 'Select the retained outcome that describes what happened, then complete Gate F.'
      : isReadyForDelivery
        ? 'Gate F is waiting for a retained outcome record.'
      : isOutcomeRecorded
  ...
  const completionPanel = isReadyForDelivery
    ? records.length
      ? `<section class="action-panel ready"><p class="panel-label">GATE F · OUTCOME COMPLETION</p><h2>Complete Gate F</h2><p>Pick the retained outcome that best represents the final result, then record it as the authoritative outcome for this Change Case.</p><label for="outcome-digest">Retained outcome record</label><select id="outcome-digest" ${canComplete ? '' : 'disabled'}>${records.map((record, index) => outcomeOption(record, index, record.outcomeDigest === selectedOutcomeDigest)).join('')}</select><div class="decision-points"><p>Gate F only accepts retained outcomes that already exist in the outcome ledger.</p><p>The browser does not invent or edit outcome records.</p></div>${canComplete ? `<button id="complete-gate-f" class="button primary" type="button" data-endpoint="${escapeHtml(completionEndpoint)}" data-expected-version="${escapeHtml(String(expectedVersion))}">Complete Gate F</button><p id="completion-status" class="live-status" role="status" aria-live="polite"></p>` : '<p class="muted">You can view the retained outcome history, but an authorized reviewer is required to complete Gate F.</p>'}</section>`
      : `<section class="action-panel blocked"><p class="panel-label">GATE F · WAITING FOR EVIDENCE</p><h2>Nothing to record yet</h2><p>Gate F records what happened after delivery. ADX has not received a retained outcome for this Change Case, so there is no decision for you to make on this screen.</p><ol class="next-steps"><li><strong>Finish Gate E</strong> — review and approve the exact delivery preview${deliveryReviewUrl ? ` <a class="text-link" href="${escapeHtml(deliveryReviewUrl)}">Open Gate E</a>` : ''}.</li><li><strong>Run the approved change</strong> in the authorized environment.</li><li><strong>Have the outcome service retain the result</strong> (success, failure, or rolled back) against this Change Case.</li><li><strong>Return here</strong> to select that retained record and complete Gate F.</li></ol><p class="muted">Activity, transcripts, and unverified release claims cannot be used to complete Gate F.</p></section>`
    : isOutcomeRecorded
