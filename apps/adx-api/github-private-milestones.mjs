...
export function createPrivateGitHubMilestoneClient({ token, fetchImpl = globalThis.fetch } = {}) {
  ...
  return Object.freeze({
    ...
    async featuresFromMilestone({ owner, repository, milestone: requestedMilestone, featureOwner, targetRepository, riskTier = 'R2' }) {
      ...
      const issues = await githubJson(fetchImpl, token, githubUrl(safeOwner, safeRepository, `/issues?state=all&milestone=${number}&per_page=${maxIssueCount}`))
      ...
      const selectedIssues = issues.filter((issue) => !issue.pull_request).slice(0, maxIssueCount).map(privateIssue)
