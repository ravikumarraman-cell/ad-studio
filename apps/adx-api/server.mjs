...
function controlPlanePage(principal, workspaces) {
  const rows = workspaces.flatMap(({ workspaceId, changeCases }) => changeCases.map((changeCase) => `<article><p class="eyebrow">${escapeHtml(changeCase.riskTier)} · ${escapeHtml(changeCase.state)}</p><h2>${escapeHtml(changeCase.title)}</h2><p><code>${escapeHtml(changeCase.id)}</code></p><nav aria-label="Review views for ${escapeHtml(changeCase.title)}"><a href="/v1/workspaces/${encodeURIComponent(workspaceId)}/change-cases/${encodeURIComponent(changeCase.id)}/story-review">Story</a><a href="/v1/workspaces/${encodeURIComponent(workspaceId)}/change-cases/${encodeURIComponent(changeCase.id)}/design-review">Design</a><a href="/v1/workspaces/${encodeURIComponent(workspaceId)}/change-cases/${encodeURIComponent(changeCase.id)}/evidence-review">Evidence</a><a href="/v1/workspaces/${encodeURIComponent(workspaceId)}/change-cases/${encodeURIComponent(changeCase.id)}/delivery-review">Delivery</a><a href="/v1/workspaces/${encodeURIComponent(workspaceId)}/change-cases/${encodeURIComponent(changeCase.id)}/outcome-review">Outcome</a></nav></article>`))
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ADX Control Plane</title><style>body{margin:0;background:#f6f8fb;color:#172033;font:16px system-ui,sans-serif;line-height:1.5}main{max-width:1080px;margin:auto;padding:32px 20px}header,section,article{background:#fff;border:1px solid #dce3ee;border-radius:14px;padding:24px;margin:16px 0}h1{margin:.2rem 0}.eyebrow{font-size:.75rem;font-weight:700;letter-spacing:.1em;color:#52657f}article{border-left:4px solid #3274c7}nav{display:flex;flex-wrap:wrap;gap:8px}a{background:#e9f4ff;color:#114c8d;padding:7px 10px;border-radius:8px;text-decoration:none;font-weight:650}code{overflow-wrap:anywhere}</style></head><body><main><header><p class="eyebrow">AUTHENTICATED ADX CONTROL PLANE</p><h1>Governed Change Cases</h1><p>Signed in as ${escapeHtml(principal.displayName ?? principal.id)}. Each view reports retained facts; no link grants approval, execution, or release authority.</p></header><section><p class="eyebrow">CHANGE CASES</p>${rows.join('') || '<p>No Change Cases are available in your workspaces yet.</p>'}</section></main></body></html>`
}
