...
import { validateDesignPackage, validateException } from './design-governance.mjs'
...
export class PostgresChangeCaseRepository {
  ...
  async transition({ scope, principal, changeCaseId, toState, expectedVersion, idempotencyKey, correlationId = randomUUID(), policyVersion = 'adx-authz-1' }) {
    ...
    return this.scoped(scope, async (client) => {
      ...
      if (toState === 'READY_FOR_EXECUTION' && ['R2', 'R3', 'R4'].includes(current.riskTier)) await this.#assertDesignGate(client, scope, changeCaseId)
      const previous = await this.#lastEvent(client, scope, changeCaseId)
  ...
  async captureDesign({ scope, principal, changeCaseId, design, expectedVersion, idempotencyKey, correlationId = randomUUID() }) { const pkg = validateDesignPackage(design); return this.scoped(scope, async (client) => { const current = await this.#currentForUpdate(client, scope, changeCaseId); if (!current || !['DESIGN_REVIEW', 'READY_FOR_EXECUTION'].includes(current.state)) throw new ChangeCaseError('DESIGN_CAPTURE_NOT_ALLOWED', 'Design package requires design review or execution-ready state.'); if (expectedVersion !== current.projectionVersion) throw new ChangeCaseError('VERSION_CONFLICT', 'The Change Case changed before this command could be applied.'); const requestDigest=sha256({changeCaseId,pkg:pkg.digest,expectedVersion}); const prior = await this.#idempotency(client, scope, idempotencyKey, requestDigest); if (prior) return { ...prior, deduplicated: true }; const now = new Date().toISOString(); const revision = Number((await client.query('SELECT COALESCE(MAX(revision),0)+1 AS revision FROM adx_design_package WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3', [changeCaseId, scope.organizationId, scope.workspaceId])).rows[0].revision); await client.query("UPDATE adx_design_approval SET status='INVALIDATED', invalidated_at=$1 WHERE change_case_id=$2 AND organization_id=$3 AND workspace_id=$4 AND status='ACTIVE'", [now, changeCaseId, scope.organizationId, scope.workspaceId]); await client.query('INSERT INTO adx_design_package (id,organization_id,workspace_id,change_case_id,revision,artifacts,design_digest,authored_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', [randomUUID(), scope.organizationId, scope.workspaceId, changeCaseId, revision, JSON.stringify(pkg.artifacts), pkg.digest, principal.id]); const previous = await this.#lastEvent(client, scope, changeCaseId); const event = createSignedEvent({ aggregateId: changeCaseId, sequence: current.projectionVersion + 1, eventType: 'ChangeCaseDesignCaptured.v1', actor: actorOf(principal), correlationId, causationId: previous.eventId, idempotencyKey, policyVersion: 'adx-design-v1', payload: { designDigest: pkg.digest, revision, toState: 'DESIGN_REVIEW' }, previousEventDigest: previous.eventDigest, signer: this.signer, occurredAt: now }); await client.query('UPDATE adx_change_case SET state=$1,projection_version=$2,updated_at=$3 WHERE id=$4 AND organization_id=$5 AND workspace_id=$6', ['DESIGN_REVIEW', event.sequence, now, changeCaseId, scope.organizationId, scope.workspaceId]); await this.#insertEvent(client,scope,event); const response={accepted:true,commandId:randomUUID(),changeCaseId,designDigest:pkg.digest,revision,newState:'DESIGN_REVIEW',projectionVersion:event.sequence,correlationId}; await this.#recordIdempotency(client,scope,idempotencyKey,requestDigest,response); await this.#outbox(client,scope,changeCaseId,event,correlationId); return response }) }
  async recordDesignException({ scope, principal, changeCaseId, designDigest, reason, expiresAt, expectedVersion, idempotencyKey, correlationId = randomUUID() }) { const exception=validateException({reason,expiresAt}); const requestDigest=sha256({command:'ChangeCaseDesignExceptionRecord.v1',changeCaseId,designDigest,exception,expectedVersion,scope}); return this.scoped(scope,async(client)=>{ const prior=await this.#idempotency(client,scope,idempotencyKey,requestDigest); if(prior)return {...prior,deduplicated:true}; const current=await this.#currentForUpdate(client,scope,changeCaseId); if(!current||current.state!=='DESIGN_REVIEW')throw new ChangeCaseError('DESIGN_EXCEPTION_NOT_ALLOWED','A design exception can be recorded only during design review.'); if(expectedVersion!==current.projectionVersion)throw new ChangeCaseError('VERSION_CONFLICT','The Change Case changed before this command could be applied.'); const pkg=await this.#currentDesignPackage(client,scope,changeCaseId); if(!pkg||pkg.designDigest!==designDigest)throw new ChangeCaseError('DESIGN_DIGEST_MISMATCH','Exception must bind to the current retained design digest.'); const now=new Date().toISOString(); const previous=await this.#lastEvent(client,scope,changeCaseId); const event=createSignedEvent({aggregateId:changeCaseId,sequence:current.projectionVersion+1,eventType:'ChangeCaseDesignExceptionRecorded.v1',actor:actorOf(principal),correlationId,causationId:previous.eventId,idempotencyKey,policyVersion:'adx-design-exception-v1',payload:{designDigest,reason:exception.reason,expiresAt:exception.expiresAt},previousEventDigest:previous.eventDigest,signer:this.signer,occurredAt:now}); await client.query("INSERT INTO adx_design_exception (id,organization_id,workspace_id,change_case_id,design_digest,reason,expires_at,requested_by,status) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'ACTIVE')",[randomUUID(),scope.organizationId,scope.workspaceId,changeCaseId,designDigest,exception.reason,exception.expiresAt,principal.id]); await client.query('UPDATE adx_change_case SET projection_version=$1,updated_at=$2 WHERE id=$3 AND organization_id=$4 AND workspace_id=$5',[event.sequence,now,changeCaseId,scope.organizationId,scope.workspaceId]); await this.#insertEvent(client,scope,event); const response={accepted:true,commandId:randomUUID(),changeCaseId,designDigest,expiresAt:exception.expiresAt,projectionVersion:event.sequence,correlationId}; await this.#recordIdempotency(client,scope,idempotencyKey,requestDigest,response); await this.#outbox(client,scope,changeCaseId,event,correlationId); return response }) }
  async decideDesign({ scope, principal, changeCaseId, designDigest, decision, rationale, expectedVersion, idempotencyKey, correlationId = randomUUID() }) { const requestDigest=sha256({command:'ChangeCaseDesignDecision.v1',changeCaseId,designDigest,decision,rationale,expectedVersion,scope}); return this.scoped(scope, async (client) => { const prior=await this.#idempotency(client,scope,idempotencyKey,requestDigest); if(prior)return {...prior,deduplicated:true}; const current=await this.#currentForUpdate(client,scope,changeCaseId); if(!current||current.state!=='DESIGN_REVIEW') throw new ChangeCaseError('DESIGN_DECISION_NOT_ALLOWED','Design review is not awaiting a decision.'); if(expectedVersion!==current.projectionVersion) throw new ChangeCaseError('VERSION_CONFLICT','The Change Case changed before this command could be applied.'); if(!['APPROVED','REJECTED'].includes(decision)||!rationale?.trim()) throw new ChangeCaseError('DESIGN_DECISION_INVALID','Decision and rationale are required.'); const pkg=await this.#currentDesignPackage(client,scope,changeCaseId); if(!pkg||pkg.designDigest!==designDigest) throw new ChangeCaseError('DESIGN_DIGEST_MISMATCH','Decision must bind to the current retained design digest.'); if(pkg.authoredBy===principal.id) throw new ChangeCaseError('APPROVAL_SEPARATION_REQUIRED','Design author cannot approve their own design.'); await this.#expireDesignExceptions(client,scope,changeCaseId,designDigest); const expired=await client.query("SELECT id FROM adx_design_exception WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 AND design_digest=$4 AND status='EXPIRED' LIMIT 1",[changeCaseId,scope.organizationId,scope.workspaceId,designDigest]); if(expired.rowCount) throw new ChangeCaseError('DESIGN_EXCEPTION_EXPIRED','An expired design exception blocks this decision.'); const previous=await this.#lastEvent(client,scope,changeCaseId); const now=new Date().toISOString(); const toState=decision==='APPROVED'?'READY_FOR_EXECUTION':'DESIGN_REVIEW'; const event=createSignedEvent({aggregateId:changeCaseId,sequence:current.projectionVersion+1,eventType:decision==='APPROVED'?'ChangeCaseDesignApproved.v1':'ChangeCaseDesignRejected.v1',actor:actorOf(principal),correlationId,causationId:previous.eventId,idempotencyKey,policyVersion:'adx-design-approval-v1',payload:{designDigest,decision,rationale:rationale.trim(),toState},previousEventDigest:previous.eventDigest,signer:this.signer,occurredAt:now}); await client.query('INSERT INTO adx_design_approval (id,organization_id,workspace_id,change_case_id,design_digest,decision,rationale,reviewed_by,status) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)',[randomUUID(),scope.organizationId,scope.workspaceId,changeCaseId,designDigest,decision,rationale.trim(),principal.id,'ACTIVE']); await client.query('UPDATE adx_change_case SET state=$1,projection_version=$2,updated_at=$3 WHERE id=$4 AND organization_id=$5 AND workspace_id=$6',[toState,event.sequence,now,changeCaseId,scope.organizationId,scope.workspaceId]); await this.#insertEvent(client,scope,event); const response={accepted:true,commandId:randomUUID(),changeCaseId,designDigest,decision,newState:toState,projectionVersion:event.sequence,correlationId}; await this.#recordIdempotency(client,scope,idempotencyKey,requestDigest,response); await this.#outbox(client,scope,changeCaseId,event,correlationId); return response }) }
  async designView(scope, changeCaseId) { return this.scoped(scope, async (client) => { const design=await this.#currentDesignPackage(client,scope,changeCaseId); if(design) await this.#expireDesignExceptions(client,scope,changeCaseId,design.designDigest); return { design:design??null, exceptions:(await client.query('SELECT design_digest AS "designDigest",reason,expires_at AS "expiresAt",status,requested_by AS "requestedBy" FROM adx_design_exception WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 ORDER BY created_at',[changeCaseId,scope.organizationId,scope.workspaceId])).rows, approvals:(await client.query('SELECT design_digest AS "designDigest",decision,rationale,reviewed_by AS "reviewedBy",status FROM adx_design_approval WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 ORDER BY created_at',[changeCaseId,scope.organizationId,scope.workspaceId])).rows } }) }
  ...
  async #currentDesignPackage(client, scope, changeCaseId) {
    return (await client.query('SELECT revision,artifacts,design_digest AS "designDigest",authored_by AS "authoredBy" FROM adx_design_package WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 ORDER BY revision DESC LIMIT 1', [changeCaseId, scope.organizationId, scope.workspaceId])).rows[0] ?? null
  }
  async #expireDesignExceptions(client, scope, changeCaseId, designDigest) {
    return client.query("UPDATE adx_design_exception SET status='EXPIRED' WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 AND design_digest=$4 AND status='ACTIVE' AND expires_at<=now() RETURNING id", [changeCaseId, scope.organizationId, scope.workspaceId, designDigest])
  }
  async #assertDesignGate(client, scope, changeCaseId) {
    const pkg = await this.#currentDesignPackage(client, scope, changeCaseId)
    if (!pkg) throw new ChangeCaseError('DESIGN_PACKAGE_REQUIRED', 'R2+ Change Cases require a complete retained design package before execution.')
    await this.#expireDesignExceptions(client, scope, changeCaseId, pkg.designDigest)
    const expired = await client.query("SELECT id FROM adx_design_exception WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 AND design_digest=$4 AND status='EXPIRED' LIMIT 1", [changeCaseId, scope.organizationId, scope.workspaceId, pkg.designDigest])
    if (expired.rowCount) throw new ChangeCaseError('DESIGN_EXCEPTION_EXPIRED', 'An expired design exception blocks execution readiness.')
    const approval = await client.query("SELECT id FROM adx_design_approval WHERE change_case_id=$1 AND organization_id=$2 AND workspace_id=$3 AND design_digest=$4 AND decision='APPROVED' AND status='ACTIVE' LIMIT 1", [changeCaseId, scope.organizationId, scope.workspaceId, pkg.designDigest])
    if (!approval.rowCount) throw new ChangeCaseError('DESIGN_APPROVAL_REQUIRED', 'R2+ Change Cases require an active independent approval bound to the current design digest.')
  }
