...
export const healthXVerificationProfile = Object.freeze({
  ...
  async verify({ page, origin }) {
    ...
    await page.getByRole('button', { name: 'You are checked in', exact: true }).waitFor()
...
async function assertProgress(page, expected) {
  assert.equal(await page.locator('[aria-label*="daily actions complete"]').getAttribute('aria-label'), expected)
}