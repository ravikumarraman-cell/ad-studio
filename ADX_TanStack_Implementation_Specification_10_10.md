...
> **10/10 upgrade rule.** This document is the normative master specification. It is intentionally split into two consumption modes: (1) a human-readable architecture contract and (2) machine-oriented requirement/stage contracts. An implementation tool MUST consume the smallest applicable bounded module plus the master invariants; it MUST NOT assume that unmentioned behavior is permitted.

---

# 0A. Normative language and requirement IDs

This document uses the following normative vocabulary:

| Keyword | Meaning |
|---|---|
| **MUST** | Mandatory for conformance. A release-capable implementation may not waive it without an explicit exception record with owner, rationale, compensating control, expiry, and approval. |
| **MUST NOT** | Prohibited. Violations are release-blocking defects unless a formally approved break-glass policy explicitly permits the action. |
| **SHOULD** | Strong default. Deviations require an ADR or implementation note explaining the trade-off. |
| **SHOULD NOT** | Strong anti-pattern. Deviations require explicit rationale and review. |
| **MAY** | Optional capability that does not weaken a MUST/MUST NOT invariant. |

Every externally observable requirement is assigned a stable ID in the canonical machine-readable registry at `docs/requirements/registry.yaml`. Requirement IDs are immutable; wording may evolve only through versioned change control. The Markdown traceability view is generated from that registry and is never the authoritative status store.

### 0A.1 Requirement ID taxonomy

```text
ARCH-*   architecture and boundaries
DOM-*    domain model and state machine
API-*    API/contract behavior
TEN-*    tenancy and authorization
WEB-*    TanStack/frontend architecture
UX-*     interaction design and accessibility
SEC-*    security and sandbox enforcement
POL-*    policy engine and decisioning
AGT-*    agent/provider adapters
CTX-*    context evidence graph
EVD-*    evidence, provenance, integrity
VER-*    verification and evaluation
REL-*    release, rollout, rollback
RES-*    resilience and reconciliation
OBS-*    observability
PERF-*   performance and capacity
OPS-*    operations, DR, runbooks
GOV-*    governance, assurance, privacy
STG-*    stage-gate obligations
```

### 0A.2 Requirement conformance rule

A requirement is **implemented** only when all of the following exist:

```text
requirement ID
  -> design/contract reference
  -> code ownership
  -> automated test or justified manual verification
...
A requirement without a linked proof artifact is **unproven**, not complete.

### 0A.3 Non-silence rule

When this specification is ambiguous, the implementation agent MUST choose the safer interpretation, surface the ambiguity in an implementation note, and avoid granting additional authority. Silence MUST NOT be interpreted as permission for a broader capability.

---

# 0B. Master invariants

The following invariants apply across every stage and technology choice. They are intentionally redundant with later sections so that bounded implementation agents can consume this section alone as the minimum safety contract.

| ID | Invariant |
|---|---|
| ARCH-001 | Control-plane authority MUST remain independent of any single agent framework, model provider, Git provider, CI provider, or deployment controller. |
| ARCH-002 | The web application MUST NOT directly access privileged databases, object storage, policy engines, provider credentials, or release APIs. |
| DOM-001 | Every Change Case state transition MUST be typed, authorized, version-checked, idempotency-aware, and auditable. |
| DOM-002 | Historical events/evidence MUST be append-only and reconstructable; current state MAY be a projection. |
| API-001 | Every side-effecting command MUST carry a client/request idempotency key and return a correlation/command identifier. |
| TEN-001 | Authorization MUST be enforced server-side and at the resource/provider boundary; UI visibility is not authorization. |
| TEN-002 | Tenant/resource isolation MUST be tested against direct-object-reference, bulk, search, cache, object-storage, export, event, and webhook paths. |
| WEB-001 | TanStack Query MUST remain the authoritative client cache for server state; local stores MUST NOT mirror authoritative server entities. |
| WEB-002 | URL state that materially affects a user-viewed or shareable decision surface MUST be typed and bookmarkable. |
| UX-001 | Every meaningful workflow state MUST expose status, timestamp/recency, safety implication, and next safe action. |
| SEC-001 | No agent capability MUST rely on prompt instructions as its security boundary. |
| SEC-002 | Capability-bearing actions MUST be enforced below the agent process by the sandbox/runtime and gateway. |
| POL-001 | Policy MUST be evaluated at every capability-bearing side effect, not only at intake. |
| AGT-001 | Provider adapters MUST declare unsupported capabilities explicitly and MUST NOT simulate them. |
| AGT-002 | Agent providers MUST be evaluated independently before gaining higher authority tiers. |
| CTX-001 | Retrieved repository/document content MUST be treated as untrusted data, with provenance and freshness. |
| EVD-001 | Material evidence MUST be content-addressed and linked to provenance metadata. |
| EVD-002 | Approval/release decisions MUST bind to the exact artifact/evidence digest set and policy version. |
| VER-001 | The independent verifier MUST NOT modify the candidate being evaluated. |
| VER-002 | Missing, stale, corrupted, or unverifiable evidence MUST never be interpreted as PASS. |
| REL-001 | Production release MUST require provenance, required evidence, policy authorization, and valid approvals. |
| REL-002 | Rollback MUST be a controlled workflow with target compatibility checks, verification, and recorded outcome. |
| RES-001 | External side effects MUST use durable outbox/inbox/reconciliation patterns or an explicitly documented equivalent. |
| RES-002 | Ambiguous external-side-effect outcomes MUST enter reconciliation, never blind retry. |
| OBS-001 | Every workflow, command, provider call, and deployment MUST be traceable through stable correlation identifiers. |
| PERF-001 | Performance budgets MUST be measured in CI/staging and monitored in production; budgets are not prose-only targets. |
| OPS-001 | Production pilots MUST have tested backup/restore, event replay, object recovery, and failure-game-day procedures. |
| GOV-001 | Every top-level assurance claim MUST have an owner, assumptions, evidence, independent challenge, and review/expiry cadence. |
| GOV-002 | An expired, disproven, or unvalidated critical assumption MUST block the dependent high-risk path. |

### 0B.1 Stage gate requirements

| ID | Stage requirement | Proof expectation |
|---|---|---|
| STG-001 | Stage 0 MUST establish deterministic build/bootstrap and framework compatibility proof. | clean-checkout build + bootstrap + framework canary |
| STG-002 | Stage 1 MUST establish authenticated, tenant-safe, server-enforced authorization. | cross-tenant and privilege-bypass suite |
| STG-003 | Stage 2 MUST establish replayable Change Case state and audit events. | state-machine + projection-rebuild tests |
| STG-004 | Stage 3 MUST establish governed intake, classification, and ambiguity handling. | risk/ambiguity acceptance suite |
| STG-005 | Stage 4 MUST establish pre-execution design/security approval boundaries. | stale-approval + policy-gate tests |
| STG-006 | Stage 5 MUST establish substrate-enforced bounded agent execution. | sandbox escape + lease/gateway tests |
| STG-007 | Stage 6 MUST establish independent, reproducible verification and evidence. | verifier-isolation + reproducibility tests |
| STG-008 | Stage 7 MUST establish preview-only Git/CI/PR delivery with reconciliation. | provider contract + duplicate/stale event tests |
| STG-009 | Stage 8 MUST establish provenance-bound progressive release and rollback. | release game-day suite |
| STG-010 | Stage 9 MUST establish outcome records and evaluation feedback without leaking sensitive evidence. | outcome/evaluation export suite |
| STG-011 | Stage 10 MUST establish context freshness/provenance and evidence-based specialist roles. | poisoning/freshness + role-value evaluation |

Stage requirements are gate predicates. A stage may not be marked `ACCEPTED` while its required stage requirement is unverified.

---

# 0C. Bounded implementation-module contract

The master file is authoritative, but agents should not be forced to reason over the entire document for every change. Implementations MUST be decomposable into bounded modules.

## 0C.1 Canonical module map

```text
M00 master invariants + repository rules
M01 domain/change-case
M02 identity/tenancy/policy
M03 API/contracts/events
M04 TanStack web shell
M05 forms/tables/query/navigation
M06 UX/accessibility/design system
M07 execution lease/sandbox/tool gateway
M08 agent/provider adapters
M09 context/evidence graph
M10 verification/evaluation
M11 provenance/audit/evidence
M12 Git/CI/release/rollout/rollback
M13 resilience/reconciliation
M14 observability/performance
M15 privacy/retention/operations/DR
M16 assurance/evaluation/governance
```

For any implementation task, the tool MUST receive:

```text
master invariants
+ applicable module(s)
+ applicable stage gate
+ repository-local instructions
+ exact acceptance tests
```

It SHOULD NOT receive unrelated modules unless a declared dependency crosses the boundary.

The files named in the module map MUST exist before Stage 0 is accepted. Each is generated or validated against the module-contract template; a module named only in prose is not a valid implementation boundary.

## 0C.2 Module contract template

Every implementation module MUST document:

```text
Module ID
Purpose
Owned entities/components
Public interfaces
...
## 0C.3 Change-impact rule

A material change to a contract, permission, policy, event schema, state transition, artifact digest model, or release rule MUST identify impacted requirements and dependent modules before code changes merge.

---

# 0D. Deterministic interpretation rules

The following rules remove common implementation ambiguity:

1. **Server truth beats client truth.** If TanStack Query, a local store, URL state, or a browser action conflicts with an authoritative backend projection, the backend projection wins and the UI reconciles.
2. **Observed external state beats requested state.** A provider request being accepted does not mean the external side effect completed.
3. **Denied beats unknown.** If authorization cannot be established, deny.
4. **Unknown safety beats automatic continuation.** If evidence, telemetry, provenance, or reconciliation is incomplete for a privileged action, pause or block.
5. **New artifact set invalidates dependent approval.** Any material change to an approved artifact/evidence set invalidates downstream approvals.
6. **Expired authority is no authority.** Lease expiry, policy expiry, approval expiry, or exception expiry blocks the corresponding action.
7. **Irreversible actions require explicit compatibility proof.** If compensation or rollback is not possible, the workflow MUST state the irreversibility and obtain the required human authority before execution.
8. **No hidden fallback.** Fallbacks that change security posture, data boundary, cost class, or release behavior MUST be explicit in policy and visible to operators.
9. **Retries are semantic, not generic.** A retry MUST be classified as read-only, idempotent side effect, or non-idempotent side effect before it is automated.
10. **Evidence age matters.** Evidence MUST carry a freshness/validity policy; stale evidence cannot silently satisfy a current gate.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) recommends calibrated autonomy, an event-sourced workflow, typed contracts, separate execution/control planes, and evaluation on organizational work rather than vendor leaderboards.
...
The current UI accepts a reviewed CSV feature file and submits it to one authenticated server-side import command, which creates one durable Change Case per accepted feature. Each feature is carried through create, intake, and risk classification with deterministic idempotency keys derived from the import ID and feature ID. The explicit import policy is `PARTIAL_SUCCESS_RESUMABLE`: a retry with the same import ID returns the existing Change Case rather than creating a duplicate, and reports `IMPORTED`, `REQUIRES_CLARIFICATION`, or `FAILED` per feature. The standard template is [adx-feature-import-template.csv](./apps/adx-studio-web/public/samples/adx-feature-import-template.csv); the three-feature example is [adx-health-insurance-features.csv](./apps/adx-studio-web/public/samples/adx-health-insurance-features.csv).

Required columns are `feature_id`, `title`, `description`, `priority`, `owner`, `target_repository`, `acceptance_criteria`, and `risk_tier`. The importer rejects duplicate feature IDs, requires `P0`–`P2` priority and `R0`–`R4` risk tiers, and requires acceptance criteria to describe an observable outcome. Optional context such as source URL, dependencies, non-functional requirements, user type, user goal, business value, out-of-scope items, and success metrics remains available to the author during review. File acceptance does not bypass Gate A, risk classification, story curation, or independent approval.
...
- **React**.
- **TanStack Start** as the full-stack React framework where deployment constraints permit it. TanStack Start is currently documented as release-candidate software, with APIs described as feature-complete/stable, so the implementation must pin exact versions and treat framework upgrades as controlled platform changes. See §42.
- **TanStack Router** with file-based routing. Its file-based routing is the preferred/recommended approach and provides typed route linkages and automatic route code splitting. See §42.
- **TanStack Query** for server state, cache, mutations, invalidation, retries and synchronization. TanStack Query's model is query + mutation + targeted invalidation rather than a hand-built normalized cache. See §42.
- **TanStack Form** for complex workflow forms and nested review/authorization forms with field-level and form-level synchronous/asynchronous validation. See §42.
- **TanStack Table** for evidence matrices, approvals, jobs, policy results, findings and audit records.
- **TanStack Virtual** for long lists and evidence-heavy views. TanStack Table does not provide virtualization itself, but the official guidance shows it can be paired with TanStack Virtual. See §42.
- **TanStack Store** only for narrow, truly client-local reactive state such as panel layout, ephemeral selection, command palette state, draft local UI preferences, and streaming-view coordination. TanStack Store is framework-agnostic and signal-oriented. See §42.
- **TanStack Devtools / package-specific devtools** in non-production development and controlled diagnostics. The Router has dedicated devtools; TanStack Devtools also exposes an extensible inspection shell. See §42.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) explicitly recommends an event-sourced system so decisions, tool calls, artifact hashes, approvals, policy results, deployments and rollbacks are immutable and attributable.
...
Do not blindly accept default retries. TanStack Query retries failed queries by default, with exponential backoff; mutations default to no retry. These defaults are useful infrastructure behavior but ADX must override them by request semantics, because retrying an idempotent read is radically different from retrying a command that changes security or deployment state. See §42.
...
After a mutation succeeds, invalidate or atomically update the affected query family. TanStack Query explicitly recommends targeted invalidation after successful mutations. See §42.
...
TanStack Form supports validation at field and form levels, synchronous or asynchronous, and user-controlled validation timing, which is appropriate for long enterprise forms with remote checks. See §42.
...
Use TanStack Virtual where row counts can exceed roughly a few hundred visible records or where cells are complex. The virtualization layer must remain independent from row semantics. TanStack's documentation specifically describes pairing Table with Virtual. See §42.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) identifies prompt injection, exfiltration, ambient secrets, supply-chain compromise, unsafe tools, confused deputy behavior, evidence forgery and runaway cost as primary threats.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) recommends a context evidence graph containing code, symbols, builds, tests, ownership, services, APIs, dependencies and history, with freshness/version checks.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) correctly replaces a simplistic 5xx-only equation with multi-metric sequential analysis, minimum samples, latency/saturation/domain guardrails, and pause/rollback behavior.
...
For stateful data changes, prefer forward-compatible recovery where rollback of the application binary is insufficient. The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) calls for expand/contract migrations, backup/restore validation, idempotent backfills and explicit rollback/roll-forward decisions.
...
### Stage conformance record

Stage 0 MUST publish requirement IDs, test commands, evidence artifact locations, owner, and current status before exit. The same pattern applies to every later stage.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) explicitly cautions that multi-agent systems should be introduced when measured value justifies their scaffolding.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) recommends historical replay, prospective shadow mode and an adversarial safety suite.
...
# 33A. Requirements traceability and release evidence matrix

This matrix is the bridge between architecture prose and executable verification. The complete registry MAY grow beyond this initial baseline, but these rows are mandatory for a 10/10 conformance claim.

| Requirement ID | Requirement | Primary design section | Proof method | Evidence artifact | Stage | Owner | Criticality |
|---|---|---|---|---|---|---|---|
| ARCH-001 | Replaceable control-plane integrations | §1, §15, §23 | adapter contract tests + second-provider test | adapter test report | 7/10 | Architecture owner | REQUIRED |
| ARCH-002 | Web cannot bypass control plane | §2, §7, §13 | negative integration test | access-denial evidence | 1/5 | Web/platform owner | REQUIRED |
| DOM-001 | Safe state transitions | §4.2 | property/state-machine tests | transition test bundle | 2/3 | Domain owner | REQUIRED |
| DOM-002 | Replayable event history | §5 | projection rebuild test | replay report | 2 | Data/platform owner | REQUIRED |
| API-001 | Command idempotency | §6.1, §12.4 | duplicate/concurrent command tests | idempotency report | 2+ | API owner | REQUIRED |
| TEN-001 | Server-side authorization | §4.1.2, §13 | bypass tests | authz evidence | 1 | Security owner | REQUIRED |
| TEN-002 | Tenant isolation | §4.1.2 | cross-tenant attack suite | tenancy test report | 1 | Security owner | REQUIRED |
| WEB-001 | TanStack Query owns server state | §7.3–7.8 | static architecture checks + browser tests | cache ownership report | 0/1 | Web owner | REQUIRED |
| WEB-002 | Typed URL state | §7.1, §29 | route/search contract tests | deep-link suite | 0/1 | Web owner | REQUIRED |
| UX-001 | Safe-state UX | §8, §9 | moderated workflow tests + browser assertions | UX evidence bundle | 1–8 | Experience owner | REQUIRED |
| SEC-001 | Prompt/runtime separation | §13.2–13.3 | adversarial injection suite | security challenge report | 5 | Security owner | REQUIRED |
| SEC-002 | Substrate enforcement | §13.2.1 | sandbox escape suite | runtime security report | 5 | Sandbox owner | REQUIRED |
| POL-001 | Side-effect policy checks | §14 | policy decision tests | policy evidence | 1/5/8 | Policy owner | REQUIRED |
| AGT-001 | Explicit provider capabilities | §15 | adapter contract tests | capability matrix | 5/10 | Agent platform owner | REQUIRED |
| AGT-002 | Provider evaluation | §31 | frozen replay + adversarial evaluation | provider scorecard | 6/10 | Evaluation owner | REQUIRED |
| CTX-001 | Untrusted content labels/freshness | §16 | retrieval poisoning/freshness tests | context audit report | 10 | Context owner | REQUIRED |
| EVD-001 | Content-addressed evidence | §5.3 | digest recomputation test | integrity report | 6 | Evidence owner | REQUIRED |
| EVD-002 | Digest-bound approvals | §6.2, §8.6 | stale-approval tests | approval attestation report | 4/6/7 | Approval owner | REQUIRED |
| VER-001 | Independent verifier immutability | §17 | write-denial tests | verifier isolation report | 6 | Verification owner | REQUIRED |
| VER-002 | Missing evidence cannot pass | §17 | negative verification tests | evidence-gating report | 6 | Verification owner | REQUIRED |
| REL-001 | Provenance-bound release | §18, §23 | mismatched artifact denial game day | release evidence | 8 | Release owner | REQUIRED |
| REL-002 | Safe rollback | §18.3 | rollback game day | rollback report | 8 | SRE/release owner | REQUIRED |
| RES-001 | Durable external side effects | §5.4, §12 | outbox/inbox tests | reconciliation report | 2/7/8 | Reliability owner | REQUIRED |
| RES-002 | No blind retry | §12.2 | timeout/duplicate tests | provider drift report | 7/8 | Integration owner | REQUIRED |
| OBS-001 | End-to-end correlation | §21 | trace continuity test | trace evidence | 0/7/8 | Observability owner | REQUIRED |
| PERF-001 | Measured budgets | §11 | performance CI + load test | perf report | 0/10 | Performance owner | REQUIRED |
| OPS-001 | Recoverability | §43.4 | restore/replay/game day | DR report | 8+ | Operations owner | REQUIRED |
| GOV-001 | Living assurance case | §43 | assurance review | signed assurance package | 10+ | Governance owner | REQUIRED |
| GOV-002 | Critical assumption blocking | §43.2 | assumption-expiry simulation | governance evidence | 8+ | Governance owner | REQUIRED |

### 33A.1 Traceability status vocabulary

```text
PLANNED      requirement recorded, implementation not started
DESIGNED     design/contract approved
IMPLEMENTED  code/config exists
VERIFIED     automated or independently reviewed proof exists
ACCEPTED     stage owner and required independent reviewer accepted evidence
EXPIRED      previous evidence no longer satisfies validity window
BLOCKED      implementation or evidence is prevented by an unmet prerequisite
WAIVED       only through explicit exception policy; never equivalent to VERIFIED
```

`WAIVED` MUST display prominently in release and assurance views. A waiver MUST NOT be counted as passing evidence.
...
The [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) establishes concrete readiness criteria for a bounded production pilot. This implementation specification turns them into executable test objectives.
...
Use this exact sequence when delegating implementation to an automation tool. The tool MUST load M00 plus the relevant bounded modules, then follow the applicable stage machine contract and trace every implementation decision to requirement IDs.
...
A feature is complete only when it can be traced from requirement to proof.

```text
[ ] requirement ID(s) assigned
[ ] domain model exists where applicable
[ ] contract exists
...
[ ] negative tests exist for security/authorization where applicable
[ ] accessibility verified
[ ] performance budget measured or explicitly not applicable
[ ] failure injection tested
[ ] requirement-to-test-to-evidence links recorded
[ ] owner assigned
[ ] documentation updated
[ ] runbook updated when operational behavior changes
[ ] no requirement is marked VERIFIED solely by a self-report
```

The feature is not **VERIFIED** until the required evidence artifact is linked. It is not **ACCEPTED** until the designated owner and independent reviewer, where required by risk tier, approve that evidence.
...
# 40A. Documentation packaging and implementation handoff

The master Markdown remains the authoritative narrative document. Before Stage 0 exit, the repository MUST contain the following synchronized artifacts. `docs/requirements/registry.yaml` is the canonical source for requirement IDs, criticality, status, proof obligations, and traceability; generated Markdown/CSV views must be checked for drift in CI.

```text
docs/requirements/registry.yaml
docs/requirements/traceability.csv
docs/stages/stages.yaml
docs/modules/M00-master-invariants.md through docs/modules/M16-assurance-governance.md
docs/assurance/claims.yaml
docs/assurance/assumptions.yaml
docs/runbooks/*.md
```

CI MUST reject a pull request when a changed requirement, module contract, stage contract, or assurance claim has not regenerated the corresponding derived documentation and traceability artifacts.

### Handoff rule for automated builders

A task package SHOULD contain only:

```text
1. repository-local instructions
2. M00 master invariants
3. one or more applicable module files
4. one applicable stage contract
5. requirement IDs in scope
6. acceptance tests
7. expected evidence
8. rollback instructions
```

The package SHOULD end with a machine-readable `done.json` or equivalent containing:

```json
{
  "status": "VERIFIED",
  "requirements": ["DOM-001", "API-001"],
  "tests": ["state_machine.spec.ts", "idempotency.spec.ts"],
  "evidence": ["sha256:...", "sha256:..."],
  "owner": "domain-platform",
  "independentReviewer": "...",
  "stage": 2,
  "nextSafeAction": "stage-3"
}
```

No task is complete when the agent merely says it is complete. The completion artifact must be independently consumable by CI, the reviewer UI, and future implementation agents.
...
The architecture is successful when the agent can move quickly **without making trust optional**. That is the governing idea of the [research blueprint](./AUTODEV_STUDIO_RESEARCH_BLUEPRINT.md) and should remain the north star for every implementation decision.
...
Assumptions use a distinct lifecycle, stored in `docs/assurance/assumptions.yaml` and surfaced in release views:

```text
PROPOSED     identified but not yet accepted as a dependency
VALID        current evidence supports the assumption within its validity window
EXPIRING     evidence remains valid but requires renewal before its stated deadline
EXPIRED      validity window elapsed; dependent high-risk paths are blocked
DISPROVEN    observed evidence contradicts the assumption; dependent paths are blocked
WAIVED       permitted only for non-release-critical assumptions with a time-bound exception
BLOCKED      validation cannot complete because a prerequisite is unavailable or unsafe
```

`VALID` requires a named evidence set, validation timestamp, validator identity, and next review date. `WAIVED` is never valid for a release-critical assumption. This lifecycle is the definition used by the release predicate in Appendix F.
...
This specification is designed to be **10/10-conformant by construction**, but it does not grant itself a product rating. The implementation earns a 10/10 operational rating only when the Appendix F release predicate is continuously satisfied and all six assurance claims have current evidence, independent challenge results, named owners, valid assumptions, and no unresolved critical finding.

The distinction is deliberate: specification quality is proven by completeness and traceability; product trust is proven by executed evidence.
...
## Appendix D — Normative requirement registry starter set

Before Stage 0 exit, the implementation team MUST create and maintain `docs/requirements/registry.yaml` as the canonical machine-readable registry and generate the Markdown traceability table from it. The source-of-truth format is intentionally tool-neutral, but every row MUST contain at least:

```yaml
id: SEC-002
statement: Sandbox capability enforcement occurs below the agent process.
level: MUST
owner: security-platform
modules: [M07]
stages: [5]
design_refs: ["13.2.1"]
verification: [sandbox_escape_suite, gateway_negative_tests]
evidence: [sandbox_security_report, gateway_receipt_bundle]
status: PLANNED
validity: permanent
...
1. IDs are immutable and unique.
2. A requirement MAY be split into child requirements, but the parent remains traceable.
3. Deleting a requirement requires a superseding requirement ID and ADR.
4. A changed requirement version invalidates dependent evidence according to its impact class.
5. Stage exit automation MUST fail when a REQUIRED requirement lacks a current proof artifact.
6. A production release MUST be impossible when a release-critical requirement is `PLANNED`, `DESIGNED`, `IMPLEMENTED`, `EXPIRED`, or `BLOCKED`.
7. `WAIVED` is never equivalent to `VERIFIED`.
8. Evidence validity windows MUST be explicit for security, policy, rollout, and operational measurements.
9. Before Stage 0 exit, CI MUST verify that every normative `MUST`/`MUST NOT` statement in the generated specification is mapped to one or more registry IDs; unmapped normative text is a documentation defect.

---

## Appendix E — Stage gate machine contract

Every implementation stage MUST be representable using the following structure so an automated builder can execute and verify it deterministically:

```yaml
stageId: 5
name: execution-lease-and-sandbox
objective: Implement bounded agent execution without ambient authority.
requires: ["STG-004", "TEN-001", "POL-001"]
modules: [M07, M08]
entryConditions:
  - prior_stage_status: ACCEPTED
  - required_dependencies_healthy: true
  - required_contract_versions_locked: true
outputs:
  - execution_lease_contract
  - sandbox_runtime
  - tool_gateway
  - security_test_bundle
requiredTests:
  - lease_validation
  - lease_expiry
  ...
  - secret_block
  - symlink_escape
  - metadata_service_block
  - process_resource_limits
requiredEvidence:
  - signed_lease_receipts
  - sandbox_security_report
  - gateway_receipt_bundle
exitCriteria:
  - all_required_tests_pass
  - all_required_evidence_verified
  - no_open_critical_security_findings
  - owner_acceptance_recorded
  - independent_review_recorded
rollback: disable stage capability, revoke active leases, preserve evidence, keep prior accepted slice operational
```

An implementation tool MUST NOT infer a stage exit from code existence alone.

---

## Appendix F — Master 10/10 acceptance equation

A production-capable ADX implementation is accepted only when:

```text
10/10 =
  every REQUIRED requirement is VERIFIED
  OR, only when it is formally non-release-critical, explicitly governed WAIVED
  AND no release-critical requirement is WAIVED
  AND all stage gates ACCEPTED
  AND all release-critical assumptions VALID
  AND all top-level assurance claims have current evidence
  AND required independent challenge has passed
  AND no critical unresolved security/reliability/integrity finding exists
  AND recovery/rollback game days are within declared objectives
  AND UX/accessibility/performance acceptance is evidenced
  AND outcome metrics show no agreed safety or trust regression
```

This is a release predicate, not a subjective quality score.
