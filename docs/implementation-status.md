...
| 5 — Leased execution and sandbox | `IN_PROGRESS` | Provider-neutral adapter declaration; signed, capability-intersected lease; tenant-scoped PostgreSQL lease/run/revocation records; append-only run events; immutable receipt storage; lease-aware gateway controls; exact egress allowlists with DNS-rebinding and private-IPv6 denial; gateway-only secret grants; and a hardened Docker runtime with retained gateway-to-runtime dispatch receipts. A live lease revocation cancels an active dispatch; output exhaustion produces `AgentRunQuotaExceeded.v1` evidence. Docker dispatches use disposable copy-on-write worktrees, retain only manifest-digested artifacts, and leave source worktrees unchanged. The local adversarial proof denies policy, symlink/hard-link/mount/archive, metadata/proxy/socket, hook, host-secret, PID-burst, output, and wall-clock escapes. | Runtime disk-quota enforcement for the disposable worktree, plus hosted-CI evidence. |
