...
Fill in these fields before execution:
...
- Non-goals:
- Acceptance criteria:
- Launch surface: `dashboard` / `tenant` / `entity` / `account` / `service` / `import` / `report` / `other`
...
Before writing code, the agent MUST produce a short reuse map with these items:
...
If the feature touches multiple domains, the agent MUST pick the narrowest owning boundary and explain why.
...
- Use the current auth/decorator model already present in the backend.
...
2. Confirm the acceptance criteria and the reuse map.
3. Decide whether the feature is an extension of an existing page or a new page; if it is a new page, define its route and Dashboard return path before coding.
4. Implement backend changes first if the feature depends on new or changed data.
5. Wire the frontend to the existing or extended API only after the backend contract is stable.
6. Add or adjust tests alongside the changed code.
7. Run the smallest useful validation command first, then expand only if needed.
8. Update docs or operational notes if the user-visible behavior changes.
...
- The final implementation notes explain exactly what was reused and why any new code was unavoidable.

## 12. Required Final Output From The Coding Agent
...
## 13. Default Instruction To The Coding Agent

If the feature request is ambiguous, stop after the discovery pass and ask for the minimum missing detail needed to choose the correct domain boundary. Do not guess a new architecture when an existing one already fits.